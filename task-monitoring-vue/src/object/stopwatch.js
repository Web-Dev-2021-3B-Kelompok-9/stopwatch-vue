...
class Stopwatch {
  constructor(textId) {
    ...
    // timerOn      : boolean
    // TimeEnd      : object of time (this class declared in ./time.js) ,
    //                to save time when stopwatch stopped
    // timeCycle    : timeout
    // textId       : string
  ...
  // ---------------- Time calculation ---------------- //
    timeMinusTime = (uT, lT) => {
      ...
      } else if (upperTime.mm - lowerTime.mm < 0) {
        upperTime.hh -= 1;
        upperTime.mm += 60;
    ...
    timePlusTime = (uT, lT) => {
      ...
      } else if (upperTime.mm + lowerTime.mm > 60) {
        upperTime.hh += 1;
        upperTime.mm -= 60;
  ...
  // ---------------- Stopwatch local storage handler ---------------- //
    autorecovery = (stopwtchTimeTextElement) => {
      ...
      // either, if the stopwatch paused, it will show the stopwatach time.
      // (not from default 00 : 00 : 00)
      ...
      const newStopwatchTime = this.timePlusTime(
        this.timeMinusTime(currentTime, timeStart), oldStopwatchTime,
      );
    ...
    // it will load stop watch time from local storage, return string of time
      JSON.parse(localStorage.getItem('timeStart'))
    ...
    updateTimeEnd = (hh, mm, ss) => {
      // update a new time end (result)
      // change hour, minute , and second with the new one (hh : mm : ss)
    ...
    setTimeText = (ArgStopwtchTimeTextElement) => {
      // to make the stopwatch cycle work for every one second
      // argument stopwtchTimeTextElement is element in page as <p> element
      //   for default format is 00 : 00 : 00
      this.Time.addOneSecond();
      // update UI stopwatch time text
      const stopwtchTimeTextElement = ArgStopwtchTimeTextElement;
    ...
    stop = (resultTimeText) => {
      ...
      this.isEnd = true;
      // reset stopwatch
      const resultText = resultTimeText;
